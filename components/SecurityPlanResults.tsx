...
const Section = ({ title, icon, children }: SectionProps) => (
    <div className="p-6 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-lg mb-6">
        <div className="flex items-center space-x-3 mb-4">
            {icon}
            <h3 className="text-xl sm:text-2xl font-semibold text-gray-700 dark:text-gray-300">{title}</h3>
        </div>
        {children}
    </div>
);
...
const SecurityPlanDetail = ({ planData }: SecurityPlanDetailProps) => {
    return (
        <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-lg shadow-lg mt-8">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-gray-700 dark:text-gray-200 mb-6">
                {planData.name}
            </h1>
            <p className="text-center text-gray-500 dark:text-gray-400 mb-6 text-sm sm:text-base">
                Creación: {planData.creation_date} | Revisión: {planData.revision_date}
            </p>

            <Section title="Identificación de Riesgos" icon={<ExclamationTriangleIcon className="w-6 sm:w-8 h-6 sm:h-8 text-red-600" />}>
                {planData.risk_identifications.map((risk: any) => (
                    <div key={risk.risk_id} className="mb-4">
                        <h4 className="text-lg sm:text-xl font-medium text-gray-800 dark:text-gray-100">🛑 {risk.description}</h4>
            ...
            <Section title="Roles y Responsabilidades" icon={<BuildingOfficeIcon className="w-6 sm:w-8 h-6 sm:h-8 text-blue-600" />}>
                {planData.roles_and_responsibilities.map((role: any) => (
                    <div key={role.role_id} className="mb-4">
                        <h4 className="text-lg sm:text-xl font-medium text-gray-800 dark:text-gray-100">👔 {role.title}</h4>
                        <p className="text-gray-600 dark:text-gray-300">Responsabilidades: {role.responsibilities}</p>
                    </div>
                ))}
            </Section>

            <Section title="Activos" icon={<ShieldCheckIcon className="w-6 sm:w-8 h-6 sm:h-8 text-green-600" />}>
                {planData.assets.map((asset: any) => (
                    <div key={asset.asset_id} className="mb-4">
                        <h4 className="text-lg sm:text-xl font-medium text-gray-800 dark:text-gray-100">🏠 {asset.asset_name}</h4>
            ...
            <Section title="Procedimientos de Respuesta a Incidentes" icon={<ClipboardDocumentCheckIcon className="w-6 sm:w-8 h-6 sm:h-8 text-yellow-600" />}>
                {planData.incident_response_procedures.map((proc: any) => (
                    <div key={proc.procedure_id} className="mb-4">
                        <h4 className="text-lg sm:text-xl font-medium text-gray-800 dark:text-gray-100">📋 {proc.title}</h4>
            ...
            <Section title="Plan de Capacitación" icon={<ShieldCheckIcon className="w-6 sm:w-8 h-6 sm:h-8 text-purple-600" />}>
                {planData.training_plan.map((training: any) => (
                    <div key={training.training_id} className="mb-4">
                        <h4 className="text-lg sm:text-xl font-medium text-gray-800 dark:text-gray-100">📚 {training.topic}</h4>
            ...
            <Section title="Políticas de Seguridad" icon={<ClipboardDocumentCheckIcon className="w-6 sm:w-8 h-6 sm:h-8 text-indigo-600" />}>
                {planData.security_policies.map((policy: any) => (
                    <div key={policy.policy_id} className="mb-4">
                        <h4 className="text-lg sm:text-xl font-medium text-gray-800 dark:text-gray-100">📜 {policy.title}</h4>
